...
- `assets/verbs/experimental/verbs_lefff.json` — Experimental list where pronominal verbs are separate entries (`lever` / `se lever`), with classification metadata. Generated by `tool/lefff/build_lefff_verbs.py` from the raw Lefff lexicon (not committed, see `tool/lefff/README.md`). Not registered in `pubspec.yaml` and not loaded by the app
